...
interface ExportXmlButtonProps {
  ...
  rows: Array<{
    ...
    BeigNMnor?: number | string | null;
  }>;
  className?: string;
}

const ExportXmlButton: React.FC<ExportXmlButtonProps> = ({ year, month, regNr, companyName, rows, className }) => {
  const handleExportXml = () => {
    const xmlRows = rows
      .map((row) => {
        const apartmentNumber = String(row.DzNumurs ?? "").trim();
        const readingValue = formatReadingValue(row.BeigMNor ?? row.BeigNMnor);
    ...
    a.href = url;
    a.download = `meter-readings-${year}-${String(month).padStart(2, "0")}.xml`;
    a.click();
