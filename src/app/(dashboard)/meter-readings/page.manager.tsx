  ...
  const ExportZipButton = dynamic(() => import("./components/ExportZipButton"), { ssr: false });
  ...
  const getReadingLogicalMeterKey = (reading: MeterReading, meterMap: Record<string, Meter>): string => {
    ...
    export default function MeterReadingsManagerPage() {
      ...
      // Получаем компанию по companyId пользователя
      useEffect(() => {
        if (user?.companyId) {
    ...
    const [exportFormat, setExportFormat] = useState<'csv' | 'xlsx' | 'xml'>('csv');
    const [exportMonth, setExportMonth] = useState<string>(''); // месяц показаний, формат YYYY-MM
    const [invoiceMonth, setInvoiceMonth] = useState<string>(''); // месяц счёта, формат YYYY-MM
    ...
    useEffect(() => {
      ...
      setInvoiceMonth(`${nextYear}-${String(nextMonth).padStart(2, '0')}`);
    }, [exportMonth]);
                      ...
                      <div className="flex flex-col gap-4 min-w-[280px]">
                        ...
                        <label className="block">
                          ...
                          <select
                            ...
                            value={exportFormat}
                            onChange={e => setExportFormat(e.target.value as 'csv' | 'xlsx' | 'xml')}
                          >
                        ...
                        <label className="block">
                          <span className="text-sm font-semibold text-gray-700 mb-2 block">Месяц показаний</span>
                          <input
                            type="month"
                            className="w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 shadow-md"
                            value={exportMonth}
                        ...
                        {exportFormat === 'xml' && (
                          <label className="block">
                            <span className="text-sm font-semibold text-gray-700 mb-2 block">Месяц счёта</span>
                            <input
                              type="month"
                              className="w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 shadow-md"
                              value={invoiceMonth}
                        ...
                        <div className="flex gap-2 mt-2">
                          {exportFormat === 'xml' && (() => {
                            if (!exportMonth || !invoiceMonth) return null;
                            const [readingYear, readingMonth] = exportMonth.split('-').map(Number);
                            const [invoiceYear, invoiceMonthNumber] = invoiceMonth.split('-').map(Number);
                            ...
                            const rows = apartmentsList.flatMap(apartment => {
                              const readings = (readingsByApartmentId[apartment.id] || []).filter(r => r.year === readingYear && r.month === readingMonth);
                              const cold = readings.find(r => meterById[r.meterId]?.name?.toLowerCase() === 'cwm');
                              const hot = readings.find(r => meterById[r.meterId]?.name?.toLowerCase() === 'hwm');
                              const apartmentRows = [] as Array<{
                                DzNumurs: string;
                                DzForeignKey: string;
                                SkdNrM: string;
                              ...
                              if (cold) {
                                const coldMeter = meterById[cold.meterId];
                                apartmentRows.push({
                                  DzNumurs: apartmentNumber,
                                  DzForeignKey: apartmentNumber,
                                  SkdNrM: getApartmentMeterSerial(apartment, coldMeter),
                                  BeigMNor: cold.currentValue,
                                });
                              }

                              if (hot) {
                                const hotMeter = meterById[hot.meterId];
                                apartmentRows.push({
                                  DzNumurs: apartmentNumber,
                                  DzForeignKey: apartmentNumber,
                                  SkdNrM: getApartmentMeterSerial(apartment, hotMeter),
                                  BeigMNor: hot.currentValue,
                                });
                              }

                              return apartmentRows;
                            });
                            const regNr = company?.registrationNumber || "";
                            const companyName = company?.name || "";
                            if (!rows.length) {
                              return (
                                <button className="px-4 py-2 rounded-lg bg-gray-300 text-gray-600 font-semibold cursor-not-allowed" disabled>
                                  Нет данных для XML
                                </button>
                              );
                            }
                            return (
                              <ExportZipButton
                                year={invoiceYear}
                                month={invoiceMonthNumber}
                                regNr={regNr}
                                companyName={companyName}
                                address={company?.address || companyName}
                                rows={rows}
                          ...
                          {exportFormat === 'csv' && (
                            <button
                              className={`px-4 py-2 rounded-lg bg-green-600 text-white font-semibold hover:bg-green-700 transition${exportRows.length === 0 ? ' opacity-50 cursor-not-allowed' : ''}`}
